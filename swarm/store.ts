...
const taskDerivedCache = new WeakMap<
  SwarmTask[],
  {
...
export function createTask(
  ...
  writeText(
    taskSpecPath(cwd, id, normalizedChannel),
    input.content?.trim()
      ? `# ${input.title}\n\n${input.content.trim()}\n`
      : `# ${input.title}\n\n*Spec pending*\n`
  );
...
export function deleteTask(cwd: string, taskId: string, channelId: string = 'general'): boolean {
  ...
  try {
    fs.unlinkSync(taskJsonPath(cwd, taskId, channelId));
  } catch {}
  try {
    fs.unlinkSync(taskSpecPath(cwd, taskId, channelId));
  } catch {}
  try {
    fs.unlinkSync(taskProgressPath(cwd, taskId, channelId));
  } catch {}
  try {
    fs.unlinkSync(taskBlockPath(cwd, taskId, channelId));
  } catch {}
...
function archiveTasks(
  ...
  for (const task of tasksToArchive) {
    ...
    moveFile(
      taskProgressPath(cwd, task.id, channelId),
      path.join(archiveTasksDir, `${task.id}.progress.md`)
    );
...
export function getReadyTasksForTasks(tasks: SwarmTask[]): SwarmTask[] {
  ...
  derived.readyTasks = tasks.filter(
    (task) => task.status === 'todo' && task.depends_on.every((dep) => doneIds.has(dep))
  );
...
export function blockTask(
  ...
  writeText(
    taskBlockPath(cwd, taskId, channelId),
...
export function resetTask(
  ...
  if (cascade) {
    for (const dependent of getTransitiveDependents(cwd, taskId, channelId)) {
      ...
      try {
        fs.unlinkSync(taskBlockPath(cwd, dependent.id, channelId));
      } catch {}
    }
  }
...
export function agentHasClaimedTask(
  cwd: string,
  agentName: string,
  channelId: string = 'general'
): boolean {
  return getTasks(cwd, channelId).some(
    (task) => task.status === 'in_progress' && task.claimed_by === agentName
  );
}
