...
export function executeSwarmStatus(cwd: string, channelId: string) {
  ...
  const lines: string[] = [`# Agent Swarm ${channelLabel}`, "", `Summary: ${summaryLine(cwd, channelId)}`, ""];
...
export function executeTask(op: string, params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
  switch (op) {
    case "create":
...
function taskCreate(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
  if (!params.title) {
...
function taskList(cwd: string, channelId: string) {
  ...
  const lines: string[] = [`# Swarm Tasks ${displayChannelLabel(channelId)}`, "", `Summary: ${summaryLine(cwd, channelId)}`, ""];
  for (const task of tasks) {
    const icon = task.status === "done" ? "✅" : task.status === "in_progress" ? "🔄" : task.status === "blocked" ? "🚫" : "⬜";
    const owner = task.claimed_by ? ` [${task.claimed_by}]` : "";
...
function taskShow(params: MessengerActionParams, cwd: string, channelId: string) {
  ...
  return result(lines.join("\n"), { mode: "task.show", channel: normalizeChannelId(channelId), task, hasProgress: !!progress });
}

function taskClaim(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskUnclaim(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskDone(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
  ...
  const completed = store.completeTask(cwd, params.id, state.agentName, summary, evidence, channelId);
...
function taskBlock(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskUnblock(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskReady(cwd: string, channelId: string) {
  ...
  const lines = [`# Ready Tasks ${displayChannelLabel(channelId)}`, "", ...ready.map(task => `- ${task.id}: ${task.title}`)];
  return result(lines.join("\n"), {
    mode: "task.ready",
    channel: normalizeChannelId(channelId),
    ready,
...
function taskProgress(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskReset(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskDelete(params: MessengerActionParams, state: MessengerState, cwd: string, channelId: string) {
...
function taskArchiveDone(state: MessengerState, cwd: string, channelId: string) {
  ...
  logFeedEvent(cwd, state.agentName, "task.archive", undefined, `${archived.archived} done task(s)`, channelId);
