...
import { formatDuration, type MessengerState, type Dirs } from './lib.js';
...
export class MessengerOverlay implements Component, Focusable {
  ...
  private calculateBasePanelHeights(
    ...
    if (this.viewState.mainView === 'tasks' && taskCount === 0) {
  ...
  private estimateFeedViewportHeight(
    ...
    const { feedHeight } = this.calculateBasePanelHeights(
      available,
      taskCount,
  ...
  private ensureFeedWindowInitialized(channelId: string, totalFeedLines: number): void {
    if (this.viewState.feedLoadedEvents.length > 0 || totalFeedLines <= 0) return;
  ...
  private syncFeedWindow(channelId: string, sectionWidth: number, totalFeedLines: number): void {
    ...
    if (totalFeedLines <= this.viewState.feedTotalLines) return;

    const previousLoadedEvents = this.viewState.feedLoadedEvents;
    const renderLastSeenTs = this.viewState.lastSeenEventTs;
    const newEvents = readFeedEventsByRange(
      this.cwd,
      this.viewState.feedTotalLines,
      totalFeedLines,
      channelId
    );
    const previousRenderedLines = this.getRenderedFeedLineCountFor(
      previousLoadedEvents,
      sectionWidth,
      renderLastSeenTs
    );
    const appendedEvents = [...previousLoadedEvents, ...newEvents];
    const appendedRenderedLines = this.getRenderedFeedLineCountFor(
      appendedEvents,
      sectionWidth,
      renderLastSeenTs
    );
    ...
    this.viewState.feedLoadedEvents = appendedEvents;

    const wasAtBottomBefore = this.viewState.wasAtBottom;

    if (this.viewState.feedLoadedEvents.length > FEED_WINDOW_SIZE) {
      const toRemove = this.viewState.feedLoadedEvents.length - FEED_WINDOW_SIZE;
    ...
    if (wasAtBottomBefore) {
      this.viewState.feedLineScrollOffset = jumpToBottom();
      return;
    }

    if (addedRenderedLines <= 0) return;

    const newRenderedLines = this.getRenderedFeedLineCountFor(
      this.viewState.feedLoadedEvents,
      sectionWidth,
      renderLastSeenTs
    );

    // Final clamping happens later in calculateVisibleRange once the exact
    // feed viewport height is known for this frame.
    this.viewState.feedLineScrollOffset = maintainScrollOnNewEvents(
  ...
  handleInput(data: string): void {
    ...
    const totalFeedLines = getFeedLineCount(this.cwd, channelId);
    const termRows = process.stdout.rows ?? 24;
    const sectionWidth = this.width - 4;
    // Note: tasks[] is declared later in this function, use a different variable name here.
    const taskList = swarmStore.getTasks(this.cwd, channelId);
    const feedHeight = this.estimateFeedViewportHeight(
      termRows,
      sectionWidth,
      taskList.length,
      totalFeedLines
    ...
    if (isFeedUpKey(data)) {
      this.viewState.pendingG = false;
      const totalRenderedLines = this.getRenderedFeedLineCount(sectionWidth);
      this.viewState.feedLineScrollOffset = scrollUp(
        this.viewState.feedLineScrollOffset,
        totalRenderedLines,
        feedHeight,
        1
      );
      this.viewState.wasAtBottom = isAtBottom(
        this.viewState.feedLineScrollOffset,
        totalRenderedLines,
        feedHeight
      );
    ...
    if (isFeedDownKey(data)) {
      this.viewState.pendingG = false;
      const totalRenderedLines = this.getRenderedFeedLineCount(sectionWidth);
      this.viewState.feedLineScrollOffset = scrollDown(this.viewState.feedLineScrollOffset, 1);
      this.viewState.wasAtBottom = isAtBottom(
        this.viewState.feedLineScrollOffset,
        totalRenderedLines,
        feedHeight
    ...
    if (data === 'g') {
      if (this.viewState.pendingG) {
        ...
        if (this.viewState.feedWindowStart > 0) {
          const newStart = 0;
          const newEnd = Math.min(FEED_GG_LOAD_SIZE, this.viewState.feedTotalLines);
          const oldestEvents = readFeedEventsByRange(this.cwd, newStart, newEnd, channelId);
          this.viewState.feedLoadedEvents = oldestEvents;
          this.viewState.feedWindowStart = newStart;
          this.viewState.feedWindowEnd = newEnd;
        }

        this.viewState.feedLineScrollOffset = jumpToTop(
          this.getRenderedFeedLineCount(sectionWidth),
          feedHeight
        );
        this.tui.requestRender();
        return;
  ...
  private generateSnapshot(): string {
    ...
    if (recentEvents.length > 0) {
      lines.push('');
      lines.push(
        `Recent: ${recentEvents.map((event) => this.formatRecentFeedEvent(event)).join(', ')}`
      );
    }
